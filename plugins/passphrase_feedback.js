...
function updateMeter(pass) {
    ...
    if (pass.length >= 15 && pass.length <= 127 && pass.match(/(.).*?((?!.*?\1).).*?((?!.*?\2).).*?((?!.*?\3).)/)) {
        $("#pass-length").removeClass("glyphicon-ok").removeClass("alert-neutral").addClass("glyphicon-ok").addClass("alert-success");
    } else {
        $("#pass-length").removeClass("glyphicon-ok").removeClass("alert-success").addClass("glyphicon-ok").addClass("alert-neutral");
    }

    // At least four words of 2+ letters each

    if (fourWords(pass)) {
        $("#pass-minwords").removeClass("glyphicon-ok").removeClass("alert-neutral").addClass("glyphicon-ok").addClass("alert-success");
    } else {
        $("#pass-minwords").removeClass("glyphicon-ok").removeClass("alert-success").addClass("glyphicon-ok").addClass("alert-neutral");
    }
    ...
    if (pass.match(/^[^>|<|"|@|#]*$/)) {
        $("#pass-symbols").removeClass("sr").addClass("sr-only");
    } else {
        $("#pass-symbols").removeClass("sr-only").addClass("sr");
    }
    ...
    if ((firstName.length <= 2 || pass.toLowerCase().indexOf(firstName) == -1) && (middleName.length <= 2 || pass.toLowerCase().indexOf(middleName) == -1) &&
        ...
        $("#pass-name").removeClass("sr").addClass("sr-only");
    } else {
         $("#pass-name").removeClass("sr-only").addClass("sr");
    }
